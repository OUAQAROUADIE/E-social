  ...
  const handleUpdate = async (attribute, value) => {
    ...
    return (
        <div className=" global-info">
            ...
            <div className=" mt-3 d-flex justify-content-between info-part ">
            <h6 className="font-weight-bold" >Work And Education</h6>
            {isEditing.workAndEducation ? (
            <input type="text" value={workAndEducation} onChange={(e) => setWorkAndEducation(e.target.value)} />
          ) : (
                    <p>{workAndEducation}</p>)}
                            {isEditing.workAndEducation ? (
                                 <a className="save-icon" onClick={() => { handleUpdate('workAndEducation', workAndEducation); handleSaveClick('workAndEducation'); }}>Save</a>
                                 ) : (
                                   <a className="edit-icon" onClick={() => handleEditClick('workAndEducation')}>
                                     <MdEdit className="edit-icon" />
                                   </a>
                                 )}
            </div>
            ...
            <input type="text" value={website} onChange={(e) => setWebsite(e.target.value)} />
                    ...
                    <p>{website}</p>)}
                            {isEditing.website ? (
                                 <a className="save-icon" onClick={() => { handleUpdate('website', website); handleSaveClick('website'); }}>Save</a>
                                 ) : (
                                   <a className="edit-icon" onClick={() => handleEditClick('website')}>
                                     <MdEdit className="edit-icon" />
                                   </a>
                                 )}
            </div>
